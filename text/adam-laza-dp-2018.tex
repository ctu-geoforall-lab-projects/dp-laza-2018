...
runs on host machine server at port 5000 as well the image built from its Dockerfile is used for every container creation. For developing 
purpose some sections were added to configuration file as well some minor changes for instance in server routing were made. The complete diff 
file is at 
...
Before the work on this thesis started, the pywps-demo project offers two dockerfiles, 
...
were some issues with \textit{Geos} and \textit{Xerces} libraries whose packages are not available for alpine distribution and their manual
instalation was necessary. The newly-created Dockerfile is available at . At the end of the work on this thesis pywps-demo offers dockerfiles
based on \textit{alpine} and \textit{ubuntu} Linux distribution.
